...
public class Permutation {
	...
	static Random ran = new Random();
 	public Permutation(int aLogLen){
		if (aLogLen > 255) {
 	...
 	public Permutation(int aLogLen, byte[] key){//J bit length of the permuation number
		if (aLogLen > 255) {
			System.out.println("Wrong Permutation");
			return;
		}
		...
		for (int i = 0; i < logLen; i++)
			length *= 2;
		keyA = new int[8];  
		for (int i = 0; i < 8; i++) keyA[i] = T.byte2int(key, i*3);
